...
{
    ofstream FILE;
    FILE.open("result_2A.txt");
    double totalTimeTaken = 0.0;

    for (int i = 0; i < TOTAL_RUNS; i++)
        totalTimeTaken += run(true);

    FILE << "Average Time Taken for sorting UD Data set using Merge Sort, ";
    FILE << TOTAL_RUNS;
    FILE << " runs ";
    ...
    FILE << "Average Time Taken for sorting ND Data set using Merge Sort, ";
    FILE << TOTAL_RUNS;
    FILE << " runs ";
    FILE << totalTimeTaken / TOTAL_RUNS;
    FILE << " seconds.\n";
